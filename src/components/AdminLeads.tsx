      ...
      <div className="min-h-screen bg-[#000000] text-[#FFFDF3] font-mono flex items-center justify-center p-6">
        <div className="w-full max-w-sm bg-[#111111] border border-white/20 p-8 space-y-6 shadow-2xl">
          <div className="space-y-1">
            <span className="text-[10px] text-white/50 uppercase tracking-widest block font-normal">[ ACCESO RESTRINGIDO ]</span>
            <h3 className="text-xl font-normal uppercase text-white font-sans">Panel de Leads</h3>
          </div>
          <form
          ...
          >
            <input
              ...
              className="w-full bg-[#000000] border border-white/20 p-3 text-xs text-white outline-none focus:border-white font-mono font-normal"
            />
            <button
              type="submit"
              disabled={loading}
              className="w-full p-3.5 bg-white text-black font-normal uppercase text-xs hover:bg-neutral-200 transition-all cursor-pointer disabled:opacity-50"
            >
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-[#FFFDF3] font-mono p-6 md:p-12 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/15 pb-6 gap-4">
        <div>
          <span className="text-xs text-white/50 uppercase tracking-widest font-normal">[ ADMIN DASHBOARD ]</span>
        ...
        <button
          ...
          className="px-4 py-2 border border-white/20 text-xs font-normal uppercase hover:bg-white/10"
        >
          Cerrar Sesión
        </button>
      </div>
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="border-b border-white/20 bg-white/5 text-white/70 uppercase">
              ...
              <th className="p-4 font-normal">Estimación Mostrada</th>
            </tr>
          </thead>
          <tbody>
            {leads.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-8 text-center text-white/40 font-normal">
              ...
              leads.map((lead) => (
                <tr key={lead.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                  <td className="p-4 text-white/60 font-normal">{new Date(lead.createdAt).toLocaleString("es-ES")}</td>
                  <td className="p-4 font-normal text-white">{lead.name || "—"}</td>
                  <td className="p-4 text-white font-normal">{lead.email}</td>
                  <td className="p-4 text-white/80 font-normal">{lead.phone}</td>
                  <td className="p-4 uppercase font-normal text-white/90">{lead.projectType}</td>
