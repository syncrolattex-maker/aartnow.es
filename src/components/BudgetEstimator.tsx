...
export default function BudgetEstimator() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-[#000000] border border-white/20 p-6 md:p-10 font-sans text-[#FFFDF3] space-y-8 shadow-2xl">
      
      {/* Indicator header */}
      <div className="space-y-4 border-b border-white/15 pb-6">
        <div className="flex justify-between items-center text-xs font-normal uppercase tracking-widest text-white/50">
      ...
      {step === 1 && (
        <div className="space-y-6">
          <h3 className="text-xl md:text-2xl font-normal uppercase text-white font-sans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {PROJECT_TYPES.map((t) => (
              ...
              >
                <div className="text-sm font-normal uppercase mb-1 flex items-center justify-between">
                  <GlitchText text={t.label} />
                  <span>→</span>
                </div>
                <div className="text-xs opacity-70 leading-relaxed font-sans font-light">
                  {t.description}
                </div>
              </button>
            ))}
            ...
            {options.map((o) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="text-xs md:text-sm font-sans">{o.label}</span>
                </label>
              );
            })}
          </div>
              ...
              return (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 accent-white cursor-pointer"
                  />
                  <span className="text-xs md:text-sm font-sans font-normal">{t.label}</span>
                </label>
              );
            })}
          </div>
      ...
      {step === 4 && estimate && (
        <div className="space-y-6">
          ...
          <div className="p-8 bg-[#111111] border border-white/20 text-center space-y-2 shadow-inner">
            <span className="text-[10px] uppercase tracking-widest text-white/50 block font-normal">
              ESTIMACIÓN ESTIMADA APROXIMADA
            </span>
            <div
              className={`text-3xl md:text-5xl font-normal font-sans text-white transition-all duration-500 ${
          ...
          {!unlocked ? (
            <form onSubmit={handleUnlock} className="space-y-4 pt-2">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-[#111111] border border-white/20 px-4 py-3 text-xs text-white font-sans outline-none focus:border-white"
                />
              ...
              <div className="flex gap-3 pt-2">
                <button 
                  type="button" 
                  onClick={() => setStep(3)} 
                  className="px-5 py-3.5 border border-white/30 text-white text-xs font-normal uppercase hover:bg-white/10"
                >
                  Atrás
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-3.5 bg-white text-black font-normal uppercase text-xs hover:bg-neutral-200 transition-all cursor-pointer shadow-xl disabled:opacity-50"
                >
            ...
            <div className="p-6 bg-white/10 border border-white/30 text-center space-y-3 font-sans">
  ...
  return (
    <div className="flex gap-4 pt-4 border-t border-white/10">
      <button
        type="button"
        onClick={onBack}
        className="px-6 py-3 border border-white/30 text-white text-xs font-normal uppercase hover:bg-white/10 transition-colors cursor-pointer"
      >
        Atrás
      </button>
      <button
        type="button"
        onClick={onNext}
        className="flex-1 py-3 bg-white text-black font-normal text-xs uppercase hover:bg-neutral-200 transition-colors cursor-pointer text-center"
      >
        {nextLabel}
      </button>
    </div>
