  ...
  return (
    <div data-sticky-bar="true" data-no-cursor-trail="true" className="fixed bottom-0 left-0 right-0 z-40 px-6 py-3.5 bg-black/60 backdrop-blur-2xl backdrop-saturate-150 border-t border-white/15 text-[11px] font-sans font-normal uppercase text-white/80 flex justify-between items-center pointer-events-auto shadow-[0_-8px_32px_rgba(0,0,0,0.5)]">
      ...
      <div className="hidden lg:flex items-center gap-2 text-white">
        <span className="text-[#A3FF12] font-normal">[</span>
        <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-ping"></span>
        <span className="font-normal tracking-widest">{time || '14:22:00'}</span>
        <span className="text-[#A3FF12] font-normal">]</span>
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className="hidden sm:flex items-center gap-4 text-white/60">
          <a href="https://www.instagram.com/aaron_primdesign/" target="_blank" rel="noreferrer" className="hover:text-[#A3FF12] transition-colors">
            instagram ↗
          </a>
          <a href="https://www.linkedin.com/in/aaron-almarche-457a6b55/" target="_blank" rel="noreferrer" className="hover:text-[#A3FF12] transition-colors">
            linkedin ↗
          </a>
        </div>
          ...
          {languages.map((item, idx) => (
            <div key={item.code} className="flex items-center gap-1">
              <button 
                ...
                className={`px-1 rounded transition-colors font-normal ${
